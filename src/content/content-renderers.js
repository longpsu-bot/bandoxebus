import { formatPresentationMetric } from '../presentation-metrics.js';
...
export function createContentRendererRegistry(context) {
  const legacy = {
    ...
    'stat-group': (block) => { const group = element(context.documentRef, 'div', 'presentation-metrics'); for (const item of block.items) { const metric = context.metrics.resolve(item.metric); const value = typeof item.format === 'string' ? formatPresentationMetric(metric.value, item.format, context.formatter) : context.formatter.format(metric.value, item.format ?? metric.format); const card = element(context.documentRef, 'article', `presentation-metric presentation-metric--${item.tone ?? 'neutral'}`); card.append(element(context.documentRef, 'span', 'presentation-metric__label', item.label), element(context.documentRef, 'strong', 'presentation-metric__value', value)); group.append(card); } return group; },
