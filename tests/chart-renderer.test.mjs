...
class Element {
  constructor(tagName) { this.tagName = tagName; this.children = []; this.attributes = {}; this.className = ''; this.textContent = ''; this.style = {}; }
...
test('chart renderer bounds the canvas height to prevent responsive resize feedback', () => {
  const table = { columns: [{ id: 'year', label: 'Year', type: 'integer' }, { id: 'value', label: 'Value', type: 'number' }], rows: [{ year: 2026, value: 20 }] };
  const renderer = createChartRenderer({ Chart: FakeChart, documentRef, reducedMotion: true, formatter: createLocaleFormatter('en-US') });
  const node = renderer.render({ type: 'chart', chartType: 'bar', title: 'Demand', data: { dataset: 'demand', x: 'year', series: [{ y: 'value', label: 'Demand' }] } }, { table });

  assert.equal(find(node, 'canvas')[0].style.maxHeight, '220px');
});
